  ...
  if (!loaded) {
    window.placeholders[`${prefix}-loaded`] = new Promise((resolve, reject) => {
      fetch(`${prefix === 'default' ? '' : prefix}/placeholders.json`)
        ...
        .catch((error) => {
          // eslint-disable-next-line no-console
          console.log(`Error fetching placeholders: ${error.message}`);
          reject();
        });
    });
  }
 ...
 * @returns an array of HTMLElement nodes that correspond to the metadata
 */
export function getAllMetadata(scope) {
  return [...document.head.querySelectorAll(`meta[property^="${scope}:"],meta[name^="${scope}-"]`)]
    .reduce((res, meta) => {
      const { property, name, content } = meta;
      const key = (property || name).split(`${scope}${scope === 'og' ? ':' : '-'}`)[1];
      res[key] = content;
      return res;
    }, {});
}
